...
class Board():
    ...
    # column = 0 : 左=nan
    # column = 3 : 右=nan
    def setParameter(self,row,column,theta):
        if row == 0:
            theta[row*4+column,0] = np.nan
            if column == 0:
                theta[row * 4+column,3] = np.nan
            elif column == 3:
                theta[row * 4+column,1] = np.nan
        elif row == 1:
            if column == 0:
                theta[row*4+column,3] = np.nan
            elif column == 3:
      ...
      # パラメータを方策に変換
    def convertParametersToMethods(self, theta):
        [m,n] = theta.shape
        methods = np.zeros((m,n))
        exp_theta = np.exp(theta)

        for i in range(0,m):
            methods[i,:] = exp_theta[i,:] / np.nansum(exp_theta[i,:])
        methods = np.nan_to_num(methods)
        return methods